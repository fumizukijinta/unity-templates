...
## 教訓ドキュメントの運用（playbook / lessons-learned）

- **`C:\Unity\templates\playbook.md`（要点版・事前読み込み用）**: 過去プロジェクトの教訓とコードパターンをカテゴリ別に抽出したもの。**実装作業の開始前に必ず読み込む**
- **`C:\Unity\templates\lessons-learned.md`（詳細記録・追記専用）**: 各実装ステップ完了時に、発生した問題と解決を**既存テンプレ形式**（4セクション構成）で「## Step N」セクションへ追記する（既存記述は消去・変更しない）
- ステップ完了時に、メインセッションが lessons-learned.md の新規事例から再発防止に効く教訓を取り出して playbook.md へ反映する（playbook は追記専用ではなく整理・統合してよい）
...
| `art-designer` | アセット棚卸し・候補出し・試着と比較スクショ | glm-5.3[1m] | 見た目の実装、購入アセットの選定・適用 |
