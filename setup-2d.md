# 2Dゲーム開発環境セットアップ手順書（Claude Code 実行用）

この手順書は、新しい**2D** Unityゲームプロジェクトの開発環境を構築するための完全な手順です。Claude Code が読んで実行します。テンプレート雛形は `C:\Unity\templates\files\` にあります。

3D版（`setup-3d.md`）との差分は **手順2のテンプレートID**、**手順3の置換値**、**手順9の疎通テスト** です。それ以外は同一の手順です。
...
| ゲームの説明（1行） | `2D横スクロールアクション` | README・CLAUDE.md |
...
## 2. プロジェクト作成（2D用テンプレート）

```powershell
# <GameName> を実際のゲーム名に、--editor-version はインストール済み Unity 6 系に置換
unity projects new <GameName> --path C:\Unity\project --editor-version 6000.6.0f1 --template com.unity.template.universal-2d
```

- **注意**: `unity projects new` は既存パスを拒否する。親（`C:\Unity\project`）は既存でも、その下の新規フォルダ名なら作成できる
- 作成されたパス: `C:\Unity\project\<GameName>`
- 2Dテンプレートは正射影カメラ・2Dライト・Sprite用URPアセットが初期設定済み
...
| `{{GAME_TYPE}}` | `2D` |
| `{{TEMPLATE_NAME}}` | `Universal 2D` |
| `{{UNITY_VERSION}}` | 実際のバージョン（例: 6000.6.0f1） |
| `{{ART_ASSET_TYPES}}` | `Sprites・Animations` |
...
git lfs install
git add -A
git commit -m "Initial commit: Unity <version> 2D URP project scaffold"
```
...
```powershell
# 2D空のGameObject生成→コンポーネント確認→削除でライブ操作を確認
unity command create_gameobject --name Player2D
unity command find_gameobjects --name Player2D
unity command delete_gameobject --target /Player2D
# SpriteRendererのアタッチが効くか（型名はアセンブリ解決後）
unity command create_gameobject --name SpriteProbe
unity command add_component --target /SpriteProbe --component UnityEngine.SpriteRenderer
unity command delete_gameobject --target /SpriteProbe
# GitHubにプッシュ（.metaの自動生成分を含む）
git add -A; git commit -m "Add scaffold files with meta"; git push
```

- `add_component` の引数が不明なら `unity command --query add_component --detail full` で確認してから実行する
...
| 2Dテンプレートが「ダウンロード可能」状態 | `unity templates list --editor <ver>` で状態確認。projects new は自動ダウンロードするが、失敗したら一度 `unity open` で空プロジェクトを開かず再試行 |
